...
bouton.addEventListener("click", () =>{   
    if (lastcitation !== aleatoire){ 
        var aleatoire = citation1[Math.floor(Math.random()*citation1.length)]
        texte.textContent = aleatoire[0]
        auteur.textContent = aleatoire[1]
    }else 
        {aleatoire = citation1[Math.floor(Math.random()*citation1.length)]
    }
    lastcitation = aleatoire;
});
